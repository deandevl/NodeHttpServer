...
class $9b6a791bf3448f12$var$HttpServer {
    ...
    callback = async (req, res)=>{
        ...
        if (Object.keys(this.middlewares).length > 0) {
            ...
            if (Object.keys(this.middlewares).includes(url_pathname)) for (let fn1 of this.middlewares[url_pathname])await fn1(req, res);
        }
        switch(method){
            case 'get':
    ...
    add_router(router) {
        ...
        for (const route1 of router.methods.post)this.methods.post[route1.route] = route1.handler;
        for (const route2 of router.methods.patch)this.methods.patch[route2.route] = route2.handler;
        for (const route3 of router.methods.delete)this.methods.delete[route3.route] = route3.handler;
    }
...
const $9b6a791bf3448f12$var$get_query_obj = (query_string)=>{
    ...
    for (const piece of query_string_pieces){
        ...
        $gSYnN$lodashset(decoded_query_string, key, data[key]);
    }
